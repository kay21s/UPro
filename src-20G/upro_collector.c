...
#if defined(MEMCPY_SSE)
	memcpy_sse2(buf->input_buf + buf->buf_length, payload_ptr, payload_len);
#elif defined(MEMCPY_C)
	memcpy_c(buf->input_buf + buf->buf_length, payload_ptr, payload_len);
#else
...
int upro_collector_read(int queue_id, int ifindex, int collector_id)
{
	int ret, i;

	upro_collector_t *cc = &(collectors[collector_id]); 
...
{
	printf("Collector on core %d, receiving ifindex:queue %d:%d ...\n", context->core_id, context->ifindex, context->queue_id);
	upro_collector_init(context);
	upro_collector_read(context->queue_id, context->ifindex, context->id);
