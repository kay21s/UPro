...
static void ixgbe_configure_rx(struct ixgbe_adapter *adapter)
{
	...
	//                0x6D5A6D5A, 0x6D5A6D5A, 0x6D5A6D5A, 0x6D5A6D5A, 
...
int ps_recv_queue(struct ps_context *context, struct ps_chunk __user *chunk_usr)
{
	...
	if (chunk.queue.qidx < 0 || chunk.queue.qidx >= adapter->num_rx_queues) {
		if (printk_ratelimit()) printk(KERN_NOTICE "Out of range: qidx %d must be in the range [%d, %d].\n",
							   chunk.queue.qidx, 0, adapter->num_rx_queues);
		return -EINVAL;
	}
		...
		unsigned int cmd, unsigned long arg)
{
	...
	*/
	...
	case PS_IOC_RECV_QUEUE:
		ret = ps_recv_queue(context, (struct ps_chunk __user *)arg);
		break;
